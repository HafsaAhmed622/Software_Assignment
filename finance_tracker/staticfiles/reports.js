...
const barIncome  = JSON.parse(document.getElementById("bar-income").textContent);
const barExpenses= JSON.parse(document.getElementById("bar-expenses").textContent);
const lineSavings= JSON.parse(document.getElementById("line-savings").textContent);
...
const donutLabels= JSON.parse(document.getElementById("donut-labels").textContent);
...
const scales = {
    x: { type: 'category', grid: { color: GRID }, ticks: { color: MUTED, maxRotation: 45 } },
...
new Chart(document.getElementById('pieChart'), {
    ...
    options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { position: 'right', labels: { color: MUTED, padding: 12, font: { size: 11 } } } }
    }
});
...
new Chart(document.getElementById('donutChart'), {
    ...
    data: {
        ...
        datasets: [{
            ...
            borderColor: '#1a1d26', borderWidth: 2, hoverOffset: 8,
        }]
    },
    options: {
        responsive: true, maintainAspectRatio: false, cutout: '65%',
        plugins: { legend: { position: 'bottom', labels: { color: MUTED, padding: 10, font: { size: 11 } } } }
    }
});